...
import styles from "./BioPTwoV1.module.css";
...
const leftPiecesTitles: string[] = ["First Station: "];

const rightPieces: JSX.Element[] = [
  <FloatingModule
    item={
      <img
        src={puzzlePiece7}
...
interface BioPTwoV1Props {}

const BioPTwoV1 = (props: BioPTwoV1Props) => {
    ...
    <div className={styles.container}>
      <div className={styles.subContainer}>
        <div className={styles.leftContainer}>
          {leftPieces.map((p: JSX.Element, idx: number) => (
            <div
              key={`Left-${idx}`}
        ...
        <div className={styles.rightContainer}>
          {rightPieces.map((p: JSX.Element, idx: number) => (
            <div
              key={`Right-${idx}`}
              className={styles.pieceContainer}
              style={
                selectedPieceId.includes(idx + 1 + 6)
                  ? { opacity: 0.5, filter: "blur(2px)" }
                  : {}
              }
              onClick={() => {
                setSelectedPieceId((prevState) => {
                  if (!selectedPieceId.includes(idx + 1 + 6)) {
                    return [...prevState, idx + 1 + 6];
                  }
                  return prevState;
                });
                setGlassModalInstance((prevState: GlassModalDataContract) => {
                  ...
                  return {
                    ...prevState,
                    content: <>{_renderPieceSwitch(idx + 1 + 6)}</>,
                    contentW: "59%",
                    contentH: "54%",
                    isOpen: true,
                  };
