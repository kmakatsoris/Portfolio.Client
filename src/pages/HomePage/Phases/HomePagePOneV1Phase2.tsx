...
import styles from "./HomePagePOneV1Phase2.module.css";
...
const MAX_PROFILE_IMAGE_SCROLL: number = 480;
const MAX_SCROLL: number = 2800;
const easeInOutQuad = (t: number) =>
  t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t;

interface HomePagePOneV1Phase2Props {
  nextPhaseProfileImage: () => void;
  prevPhaseProfileImage: () => void;
  nextPhaseDrones: () => void;
  prevPhaseDrones: () => void;
}

const HomePagePOneV1Phase2 = (props: HomePagePOneV1Phase2Props) => {
  const siteRef = useRef(null!);
  const [scrollProfileImageY, setScrollProfileImageY] = useState<number>(0);
  ...
  const movementProfileImageStyle = (scrollY: number) => {
    const progress = Math.min(scrollY / MAX_SCROLL, 1);
    const translateX =
      scrollY >= MAX_SCROLL ? -20 : 100 - easeInOutQuad(progress) * 100;
    // console.log(`Now: ${translateX}, ${100 - easeInOutQuad(progress) * 10000}`);
    return {
      ...
      <div className={styles.rightContainer}>
        <div className={styles.rightContentContainer}>
          <DefaultParallax
            ...
            initScroll={0}
            maxScroll={MAX_SCROLL}
            movementStyle={movementProfileImageStyle}
            setScrollY={setScrollProfileImageY}
            scrollY={scrollProfileImageY}
            nextPhase={props.nextPhaseProfileImage}
            prevPhase={props.prevPhaseProfileImage}
          />
        </div>
      </div>
    </div>
...
export default HomePagePOneV1Phase2;
