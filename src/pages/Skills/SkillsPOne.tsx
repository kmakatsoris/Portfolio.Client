...
import styles from "./SkillsPOne.module.css";
...
interface SkillsPOneProps {
  insidePageRefs: React.RefObject<HTMLDivElement>[];
}

const SkillsPOne = (props: SkillsPOneProps) => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.previewsContainer}>
        {Array.from({ length: PREVIEW_ROWS }, (_, i: number) => i).map(
          (idx: number) => {
            return (
              <div key={idx} className={styles[`previewRow${idx}`]}>
                {Array.from(
                  { length: PREVIEW_COLUMNS },
                  (_, j: number) => j
                ).map((j_idx: number) => {
                  // console.log( isNext * (PREVIEW_ROWS * PREVIEW_COLUMNS) + idx * PREVIEW_ROWS + j_idx );
                  return (
                    <div
                      key={j_idx}
                      className={styles[`previewColumn${j_idx}`]}
                    >
                      {skillsArray.length >
                      isNext * (PREVIEW_ROWS * PREVIEW_COLUMNS) +
                        idx * PREVIEW_ROWS +
                        j_idx ? (
                        <ButtonV2
                          title={
