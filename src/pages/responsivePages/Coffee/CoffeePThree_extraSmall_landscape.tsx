...
import styles from "./CoffeePThree_extraSmall_landscape.module.css";
...
import AddReviewDefault_extraSmall from "../../../modules/components/presentationViewer/responsive/AddReviewDefault_extraSmall";

interface CoffeePThree_extraSmall_landscapeProps {
  currentPath: string;
}

const CoffeePThree_extraSmall_landscape = (
  props: CoffeePThree_extraSmall_landscapeProps
) => {
  const {
    setCurrentPathname,
  ...
  useEffect(() => {
    setCurrentPathname(props.currentPath);
    if (grades.length === 0) {
      fetchGrades();
    }
  ...
  const _handleClickOnAddButton = () => {
    setGlassModalInstance((prevState: GlassModalDataContract) => {
      ...
      return {
        ...
        content: (
          <>
            <AddReviewDefault_extraSmall grades={grades} />
          </>
        ),
...
export default CoffeePThree_extraSmall_landscape;
